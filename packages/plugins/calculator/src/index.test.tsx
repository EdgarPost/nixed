...
describe('CalculatorPlugin', () => {
  ...
  describe('onSearch', () => {
    it('should return empty array for empty query', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '' }) as any[];
      expect(results).toEqual([]);
    });

    it('should return empty array for non-math query', () => {
      const results = CalculatorPlugin.onSearch?.({ text: 'hello world' }) as any[];
      expect(results).toEqual([]);
    });

    it('should return result for valid math expression', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '2 + 2' }) as any[];
    ...
    it('should return result for expression starting with =', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '=10 * 5' }) as any[];
      expect(results).toHaveLength(1);
      expect(results[0].title).toBe('50');
    });

    it('should return empty array for invalid expression', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '2 +' }) as any[];
      expect(results).toEqual([]);
    });

    it('should include copy actions', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '5 * 6' }) as any[];
    ...
    it('should execute copy action', async () => {
      const { clipboard, ui } = await import('@nixed/sdk');
      const results = CalculatorPlugin.onSearch?.({ text: '10 / 2' }) as any[];
    ...
    it('should execute copy expression action', async () => {
      const { clipboard, ui } = await import('@nixed/sdk');
      const results = CalculatorPlugin.onSearch?.({ text: '8 + 2' }) as any[];
  ...
  describe('isCalculationQuery', () => {
    it('should detect expressions with numbers', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '123' }) as any[];
      expect(results.length).toBeGreaterThan(0);
    });

    it('should detect expressions with operators', () => {
      const results1 = CalculatorPlugin.onSearch?.({ text: '5+5' }) as any[];
      expect(results1.length).toBeGreaterThan(0);

      const results2 = CalculatorPlugin.onSearch?.({ text: '10-2' }) as any[];
      expect(results2.length).toBeGreaterThan(0);

      const results3 = CalculatorPlugin.onSearch?.({ text: '3*4' }) as any[];
      expect(results3.length).toBeGreaterThan(0);

      const results4 = CalculatorPlugin.onSearch?.({ text: '8/2' }) as any[];
      expect(results4.length).toBeGreaterThan(0);
    });

    it('should detect expressions with parentheses', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '(2+3)*4' }) as any[];
      expect(results.length).toBeGreaterThan(0);
    });

    it('should detect expressions starting with =', () => {
      const results = CalculatorPlugin.onSearch?.({ text: '=100' }) as any[];
      expect(results.length).toBeGreaterThan(0);
    });
  });
});
